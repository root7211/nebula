...
| **Phase 5.2-R5-S1 状态绑定 L2** | `counter_demo.nelua` — L2 声明式状态绑定 demo（`states/bindings/events` in `nebula_app()` + `nebula_main()`），L0 版 139 行→L2 版 49 行（2.8x 压缩）；验证 Phase 5.0 全知图在 L2 sugar 下端到端可用；41 条冒烟测试 | `pending` |
