...
> 当前仓库的**主线能力**已完成到 **Phase 2.5**。与此同时，**Phase 3.1–3.5、Phase 3.6.x（Gap Buffer + 命中测试 + 文本选区）、Phase 3.7（管线收敛与死代码清理）与 **Phase 3.8（渲染循环封装与 FrameArena 内嵌）已全部合入仓库**。Phase 3.8 新增 `nebula_frame_render` 泛型封装函数，将 20 行 WGPU 渲染样板收缩为 1 行调用；`app_factory` 自动将 2MB FrameArena 内嵌进 App record，开发者无需手动声明后备内存。

---

## 各阶段演进

| 项目 | Phase 0–2.5 | Phase 3.1 | Phase 3.2.x | Phase 3.3.x | Phase 3.4.x | Phase 3.5.x | Phase 3.6.x | Phase 3.7 | Phase 3.8 |
|---|---|---|---|---|---|---|---|---|---|
| 核心推导 | 形状 → 状态机/管线 | 静态 Flexbox 布局 | SDF 文本子系统 | 运行时动态列表与实例渲染 | 键盘输入与单行 Input 组件 | 编译期显式编排与全面 Instancing 化 | **完整文本编辑交互：Gap Buffer + 选区支持** | **管线收敛：3 着色器 + 3 路径，~630 行死代码** | **渲染循环封装：nebula_frame_render + FrameArena 内嵌** |
| 渲染技术 | SDF 形状 + 多 Pass 阴影 | 布局对齐 | SDF 文本渲染 | Storage Buffer + Instanced 渲染 | 键盘事件流转 + 极简光标渲染 | Standard Instanced Pipeline + 类型分组批量绘制 | **L1/L2 分层 + 栈上即时排版，选区双锤点同步** | **standard_instanced 默认路径，阴影着色器使用真实 SDF** | **20 行 WGPU 样板 → 1 行；FrameArena 内嵌，无堆分配** |
| 质量保障 | 手动验证 | 同左 | 31 项 Lua 断言 + 11 项编译回归 | 111 项断言 | 12/12 测试套件，全量回归集成 | 16/16 测试套件，150+ 项断言 | **19/19 套件，260+ 项断言** | **20/20 套件，smoke_phase3_7 专项** | **21/21 套件，smoke_phase3_8 专项验证** |
...
│   │   ├── app_factory.lua         # ★ Phase 3.8: 编排工厂 v0.2，Arena 内嵌支持
│   │   └── ... (其他派生模块)
│   └── ...
├── assets/
...
│   ├── run_all_tests.sh            # ★ Phase 3.8: 全量回归测试运行器（21 项测试套件）
│   └── ...
├── tests/
...
│   └── form_demo.nelua             # ★ Phase 3.8: 表单演示（nebula_frame_render 主循环收缩到 3 行）
├── docs/
