...
Nebula 的核心范式是**编译期消解**——开发者写的每一行声明式 UI 描述，都在编译阶段被 Nelua 宏系统完整展开为确定性的原生代码，运行时不存在虚表、反射、事件总线或任何间接层：

- **阶段封闭性**：每个操作严格归属于其输入最早全部可知的阶段（S0 预处理 → S1 编译 → S2 运行），后一阶段不得执行前一阶段的操作。
- **零运行时抽象**：布局解算、状态机构建、管线签名、交互逻辑编织——全部在 S1 编译期完成。S2 运行时只剩 GPU 调用和输入响应。
- **形即渲染**：Visual 类型在编译期确定性映射到 GPU 管线签名 `Σ(V) = (VertexLayout, UniformsLayout, ShaderModule, BlendState)`，无运行时分发。

实际效果：一个完整的文本编辑器（语法高亮 + 搜索替换 + Undo/Redo + 行号 + 状态栏）只需 **19 行声明**，编译器自动推导出等价于 886 行手写代码的原生二进制。

尽管实现路径不同，Nebula 具备主流 GUI 框架应有的工程能力：

- **跨平台**：源码级支持 Linux (X11/Wayland)、Windows、Web (WebAssembly + WebGPU)。
- **GPU 原生渲染**：wgpu-native v29 SDF 矢量渲染引擎，CJK 文本排版，6 语言语法高亮。
- **完整组件体系**：按钮、表单、滚动容器、下拉菜单、多行编辑器、终端模拟器、代码浏览器——均由编译期原语组合生成。
- **声明式动画**：可编程 easing 注册表 + per-property 动画 override + delay 支持。
