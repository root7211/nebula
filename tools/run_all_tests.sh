...
# ★ Phase 4.7-S1: CJK multiline editable 升级（UTF-8 aware gap buffer + CJK 显示宽度）
...
# ★ Phase 4.7-S3: 行号显示 + 多列布局（flex_grow + flex_basis + DenseText 布局）
...
# ★ Phase 4.7-S4: 语法高亮架构验证（规则注册 + 扫描函数生成 + 关键字/注释/字符串/数字着色）
...
# ★ Phase 4.8-S5: 自动缩进 + Tab 处理（Tab 4 空格 + Shift+Tab 反缩进 + Enter 保持缩进）
...
# ★ Phase 5.2 R2: 多 App / 多 Visual 场景隔离测试
run_test "smoke_phase5_2_r2_multi (Phase 5.2 R2 — 多 App/Visual registry 隔离)" \
  nelua-lua tests/smoke_phase5_2_r2_multi.lua
...
# ★ Phase 4.7-S1: cjk_editor_demo 编译回归（CJK + ASCII 混排多行编辑器）
run_test "compile cjk_editor_demo (Phase 4.7-S1 — CJK multiline editor)" \
  nelua -c -L src -L assets/generated --add-path . --add-path examples --add-path assets/generated \
  --cflags="-I./vendor/wgpu-native/include" \
  --ldflags="-L./vendor/wgpu-native/lib -lwgpu_native -lglfw -lm -ldl -Wl,-rpath,./vendor/wgpu-native/lib" \
...
# ★ Phase 4.7-S3: editor_with_lines_demo 编译回归（行号栏 + 编辑区多列布局 + 双 DenseText）
run_test "compile editor_with_lines_demo (Phase 4.7-S3 — line numbers + multi-column DenseText layout)" \
  nelua -c -L src -L assets/generated --add-path . --add-path examples --add-path assets/generated \
  --cflags="-I./vendor/wgpu-native/include" \
  --ldflags="-L./vendor/wgpu-native/lib -lwgpu_native -lglfw -lm -ldl -Wl,-rpath,./vendor/wgpu-native/lib" \
...
# ★ Phase 4.7-S4: highlight_editor_demo 编译回归（语法高亮 + DenseText + 多列布局）
run_test "compile highlight_editor_demo (Phase 4.7-S4 — syntax highlighting + DenseText editor)" \
  nelua -c -L src -L assets/generated --add-path . --add-path examples --add-path assets/generated \
  --cflags="-I./vendor/wgpu-native/include" \
  --ldflags="-L./vendor/wgpu-native/lib -lwgpu_native -lglfw -lm -ldl -Wl,-rpath,./vendor/wgpu-native/lib" \
