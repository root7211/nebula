...
NEBULA_PRIMITIVES["multiline_editable"] = {
  ...
  process_body = function(spec, lines)
    ...
    -- delete selection: inline block
    table.insert(lines, "      -- delete from end row backwards to start row")
    table.insert(lines, "      if _ml_sel_sr == _ml_sel_er then")
    table.insert(lines, "        -- single row: delete bytes [sel_sc, sel_ec)")
    table.insert(lines, "        local _dline = self.visual.multi_buf:get_line(_ml_sel_sr)")
    table.insert(lines, "        _dline:delete_range(_ml_sel_sc, _ml_sel_ec)")
    table.insert(lines, "      else")
    table.insert(lines, "        -- multi-row deletion")
    table.insert(lines, "        -- 1. Keep start row content before sel_sc")
    table.insert(lines, "        -- 2. Keep end row content after sel_ec")
    table.insert(lines, "        -- 3. Append end-row remainder to start row")
    table.insert(lines, "        -- 4. Remove rows (sel_sr+1 .. sel_er)")
    table.insert(lines, "        local _eline = self.visual.multi_buf:get_line(_ml_sel_er)")
    table.insert(lines, "        local _eflat: [512]uint8")
    table.insert(lines, "        local _elen = _eline:flatten(&_eflat[0], 511)")
    table.insert(lines, "        -- Truncate start row at sel_sc")
    table.insert(lines, "        local _sline2 = self.visual.multi_buf:get_line(_ml_sel_sr)")
    table.insert(lines, "        local _slen2 = (@uint32)(_sline2:len())")
    table.insert(lines, "        if _ml_sel_sc < _slen2 then")
    table.insert(lines, "          _sline2:delete_range(_ml_sel_sc, _slen2)")
    table.insert(lines, "        end")
    table.insert(lines, "        -- Append end-row remainder (after sel_ec)")
    table.insert(lines, "        local _ae: uint32 = _ml_sel_ec")
    table.insert(lines, "        while _ae < (@uint32)(_elen) do")
    table.insert(lines, "          _sline2:insert_char(_eflat[_ae]); _ae = _ae + 1")
    table.insert(lines, "        end")
    table.insert(lines, "        -- Remove rows [sel_sr+1 .. sel_er] by shifting")
    table.insert(lines, "        local _del_count: uint32 = _ml_sel_er - _ml_sel_sr")
    table.insert(lines, "        local _shift_r: uint32 = _ml_sel_sr + 1")
    table.insert(lines, "        while _shift_r + _del_count < self.line_count do")
    table.insert(lines, "          local _dst = self.visual.multi_buf:get_line(_shift_r)")
    table.insert(lines, "          local _src = self.visual.multi_buf:get_line(_shift_r + _del_count)")
    table.insert(lines, "          _dst:clear()")
    table.insert(lines, "          local _stmp: [256]uint8")
    table.insert(lines, "          local _sn = _src:flatten(&_stmp[0], 255)")
    table.insert(lines, "          local _sk: uint16 = 0")
    table.insert(lines, "          while _sk < _sn do _dst:insert_char(_stmp[_sk]); _sk = _sk + 1 end")
    table.insert(lines, "          _shift_r = _shift_r + 1")
    table.insert(lines, "        end")
    table.insert(lines, "        self.line_count = self.line_count - _del_count")
    table.insert(lines, "        self.visual.multi_buf.line_count = self.line_count")
    table.insert(lines, "      end")
    table.insert(lines, "      self.cursor_row = _ml_sel_sr; self.cursor_col = _ml_sel_sc")
    ...
    table.insert(lines, "      if _ml_has_sel then")
    table.insert(lines, "        -- delete selection before paste (same as cut)")
    table.insert(lines, "        if _ml_sel_sr == _ml_sel_er then")
    table.insert(lines, "          local _dline = self.visual.multi_buf:get_line(_ml_sel_sr)")
    table.insert(lines, "          _dline:delete_range(_ml_sel_sc, _ml_sel_ec)")
    table.insert(lines, "        else")
    ...
    table.insert(lines, "          local _sline2 = self.visual.multi_buf:get_line(_ml_sel_sr)")
    table.insert(lines, "          local _slen2 = (@uint32)(_sline2:len())")
    table.insert(lines, "          if _ml_sel_sc < _slen2 then _sline2:delete_range(_ml_sel_sc, _slen2) end")
    table.insert(lines, "          local _ae: uint32 = _ml_sel_ec")
    table.insert(lines, "          while _ae < (@uint32)(_elen) do _sline2:insert_char(_eflat[_ae]); _ae = _ae + 1 end")
    table.insert(lines, "          local _del_count: uint32 = _ml_sel_er - _ml_sel_sr")
    table.insert(lines, "          local _shift_r: uint32 = _ml_sel_sr + 1")
    table.insert(lines, "          while _shift_r + _del_count < self.line_count do")
    table.insert(lines, "            local _dst = self.visual.multi_buf:get_line(_shift_r)")
    ...
    table.insert(lines, "    -- delete selection before inserting")
    table.insert(lines, "    if _ml_sel_sr == _ml_sel_er then")
    table.insert(lines, "      self.visual.multi_buf:get_line(_ml_sel_sr):delete_range(_ml_sel_sc, _ml_sel_ec)")
    table.insert(lines, "    else")
    table.insert(lines, "      local _eline = self.visual.multi_buf:get_line(_ml_sel_er)")
    table.insert(lines, "      local _eflat: [512]uint8")
    table.insert(lines, "      local _elen = _eline:flatten(&_eflat[0], 511)")
    ...
    table.insert(lines, "    if _ml_has_sel then")
    -- Delete selection before enter (inline)
    table.insert(lines, "      if _ml_sel_sr == _ml_sel_er then")
    table.insert(lines, "        self.visual.multi_buf:get_line(_ml_sel_sr):delete_range(_ml_sel_sc, _ml_sel_ec)")
    table.insert(lines, "      else")
    table.insert(lines, "        local _eline = self.visual.multi_buf:get_line(_ml_sel_er)")
