...
-- ===== multiline_editable: 删除选区辅助函数 =====
-- 将 "删除选区" 代码块注入到 lines 表中。
-- 调用方必须保证 _ml_has_sel, _ml_sel_sr/sc/er/ec 已在作用域中声明。
-- indent: 缩进字符串（如 "    " 或 "      "），用于嵌套层级。
local function _emit_ml_delete_selection(lines, indent)
  local I = indent
  local function L(s) table.insert(lines, I .. s) end
  L("if _ml_sel_sr == _ml_sel_er then")
  L("  self.visual.multi_buf:get_line(_ml_sel_sr):delete_range(_ml_sel_sc, _ml_sel_ec)")
  L("else")
  L("  local _eline = self.visual.multi_buf:get_line(_ml_sel_er)")
  L("  local _eflat: [512]uint8")
  L("  local _elen = _eline:flatten(&_eflat[0], 511)")
  L("  local _sline2 = self.visual.multi_buf:get_line(_ml_sel_sr)")
  L("  local _slen2 = (@uint32)(_sline2:len())")
  L("  if _ml_sel_sc < _slen2 then _sline2:delete_range(_ml_sel_sc, _slen2) end")
  L("  local _ae: uint32 = _ml_sel_ec")
  L("  while _ae < (@uint32)(_elen) do _sline2:insert_char(_eflat[_ae]); _ae = _ae + 1 end")
  L("  local _del_count: uint32 = _ml_sel_er - _ml_sel_sr")
  L("  local _shift_r: uint32 = _ml_sel_sr + 1")
  L("  while _shift_r + _del_count < self.line_count do")
  L("    local _dst = self.visual.multi_buf:get_line(_shift_r)")
  L("    local _src = self.visual.multi_buf:get_line(_shift_r + _del_count)")
  L("    _dst:clear()")
  L("    local _stmp: [256]uint8")
  L("    local _sn = _src:flatten(&_stmp[0], 255)")
  L("    local _sk: uint16 = 0")
  L("    while _sk < _sn do _dst:insert_char(_stmp[_sk]); _sk = _sk + 1 end")
  L("    _shift_r = _shift_r + 1")
  L("  end")
  L("  self.line_count = self.line_count - _del_count")
  L("  self.visual.multi_buf.line_count = self.line_count")
  L("end")
  L("self.cursor_row = _ml_sel_sr; self.cursor_col = _ml_sel_sc")
...
local function _emit_ml_cursor_move(lines, key, direction, indent, update_anchor)
  ...
  if direction == "left" then
    L("  if self.cursor_col > 0 then")
    L("    self.cursor_col = self.cursor_col - 1")
    L("  elseif self.cursor_row > 0 then")
    L("    self.cursor_row = self.cursor_row - 1")
    L("    self.cursor_col = (@uint32)(self.visual.multi_buf:get_line(self.cursor_row):len())")
    L("  end")
  elseif direction == "right" then
    L("  local _ml_llen = (@uint32)(self.visual.multi_buf:get_line(self.cursor_row):len())")
    L("  if self.cursor_col < _ml_llen then")
    L("    self.cursor_col = self.cursor_col + 1")
    L("  elseif self.cursor_row + 1 < self.line_count then")
    L("    self.cursor_row = self.cursor_row + 1")
    L("    self.cursor_col = 0")
    L("  end")
  elseif direction == "up" then
    L("  if self.cursor_row > 0 then")
    L("    self.cursor_row = self.cursor_row - 1")
    L("    local _ml_llen = (@uint32)(self.visual.multi_buf:get_line(self.cursor_row):len())")
    L("    if self.cursor_col > _ml_llen then self.cursor_col = _ml_llen end")
    L("  end")
  elseif direction == "down" then
    L("  if self.cursor_row + 1 < self.line_count then")
    L("    self.cursor_row = self.cursor_row + 1")
    L("    local _ml_llen = (@uint32)(self.visual.multi_buf:get_line(self.cursor_row):len())")
    L("    if self.cursor_col > _ml_llen then self.cursor_col = _ml_llen end")
    L("  end")
...
NEBULA_PRIMITIVES["multiline_editable"] = {
  ...
  context_fields = {
    ...
    -- ★ Phase 4.8 S1: 选区锚点（selection anchor）
    { name = "sel_anchor_row", type = "uint32" },
    { name = "sel_anchor_col", type = "uint32" },
  },
  context_init = {},
  process_body = function(spec, lines)
    -- ★ multiline_editable: 直接操作 multi_buf（每行是独立的 NebulaBuf256 gap buffer）
    -- ★ Phase 4.8 S1: 增加选区（selection）+ 系统剪贴板支持
    -- 字符输入、Backspace、Delete、Enter、Left/Right、Home/End、Shift+方向、Ctrl+C/V/X/A 全部在这里处理。

    -- ---- Helper: _ml_has_selection ----
    table.insert(lines, "  -- ★ Phase 4.8 S1: selection helpers")
    table.insert(lines, "  local _ml_has_sel = (self.sel_anchor_row ~= self.cursor_row) or (self.sel_anchor_col ~= self.cursor_col)")

    -- ---- Helper: _ml_delete_selection — delete selected text, returns true if anything was deleted ----
    -- This is inlined as a block that can be used before char input, backspace, delete, enter, paste
    -- After deletion, cursor is at the start of the selection, anchor is reset.
    -- We emit a helper function-like block using a do...end scope.

    -- ---- Helper function: compute ordered selection bounds ----
    table.insert(lines, "  local _ml_sel_sr: uint32 = 0")
    table.insert(lines, "  local _ml_sel_sc: uint32 = 0")
    table.insert(lines, "  local _ml_sel_er: uint32 = 0")
    table.insert(lines, "  local _ml_sel_ec: uint32 = 0")
    table.insert(lines, "  if _ml_has_sel then")
    table.insert(lines, "    if self.sel_anchor_row < self.cursor_row or (self.sel_anchor_row == self.cursor_row and self.sel_anchor_col < self.cursor_col) then")
    table.insert(lines, "      _ml_sel_sr = self.sel_anchor_row; _ml_sel_sc = self.sel_anchor_col")
    table.insert(lines, "      _ml_sel_er = self.cursor_row; _ml_sel_ec = self.cursor_col")
    table.insert(lines, "    else")
    table.insert(lines, "      _ml_sel_sr = self.cursor_row; _ml_sel_sc = self.cursor_col")
    table.insert(lines, "      _ml_sel_er = self.sel_anchor_row; _ml_sel_ec = self.sel_anchor_col")
    table.insert(lines, "    end")
    table.insert(lines, "  end")

    -- ================================================================
    -- ★ Phase 4.8 S1: Ctrl+C / Ctrl+X — Copy / Cut selected text to system clipboard
    -- ================================================================
    table.insert(lines, "  -- ★ Phase 4.8 S1: clipboard copy/cut")
    table.insert(lines, "  if (input.key_pressed == NebulaKey.Copy or input.key_pressed == NebulaKey.Cut) and _ml_has_sel then")
    table.insert(lines, "    -- serialize selected text to buffer")
    table.insert(lines, "    local _clip: [4096]uint8")
    table.insert(lines, "    local _clip_len: uint32 = 0")
    table.insert(lines, "    local _sr = _ml_sel_sr")
    table.insert(lines, "    while _sr <= _ml_sel_er and _clip_len < 4090 do")
    table.insert(lines, "      local _sline = self.visual.multi_buf:get_line(_sr)")
    table.insert(lines, "      local _sflat: [512]uint8")
    table.insert(lines, "      local _slen = _sline:flatten(&_sflat[0], 511)")
    table.insert(lines, "      local _col_start: uint32 = (_sr == _ml_sel_sr) and _ml_sel_sc or 0")
    table.insert(lines, "      local _col_end: uint32 = (_sr == _ml_sel_er) and _ml_sel_ec or (@uint32)(_slen)")
    table.insert(lines, "      local _ci: uint32 = _col_start")
    table.insert(lines, "      while _ci < _col_end and _clip_len < 4090 do")
    table.insert(lines, "        _clip[_clip_len] = _sflat[_ci]; _clip_len = _clip_len + 1; _ci = _ci + 1")
    table.insert(lines, "      end")
    table.insert(lines, "      if _sr < _ml_sel_er and _clip_len < 4090 then")
    table.insert(lines, "        _clip[_clip_len] = 10; _clip_len = _clip_len + 1")  -- newline
    table.insert(lines, "      end")
    table.insert(lines, "      _sr = _sr + 1")
    table.insert(lines, "    end")
    table.insert(lines, "    _clip[_clip_len] = 0")
    table.insert(lines, "    glfwSetClipboardString(nilptr, (@cstring)(&_clip[0]))")

    -- If Cut, delete the selection
    table.insert(lines, "    if input.key_pressed == NebulaKey.Cut then")
    -- delete selection: use helper function
    _emit_ml_delete_selection(lines, "      ")
    table.insert(lines, "    end")
    table.insert(lines, "  end")

    -- ================================================================
    -- ★ Phase 4.8 S1: Ctrl+V — Paste from system clipboard (with newline handling)
    -- ================================================================
    table.insert(lines, "  -- ★ Phase 4.8 S1: clipboard paste")
    table.insert(lines, "  if input.key_pressed == NebulaKey.Paste then")
    table.insert(lines, "    local _pclip = glfwGetClipboardString(nilptr)")
    table.insert(lines, "    if _pclip ~= nilptr then")
    -- If there is a selection, delete it first (reuse cut logic)
    table.insert(lines, "      if _ml_has_sel then")
    -- delete selection before paste: use helper function
    _emit_ml_delete_selection(lines, "        ")
    table.insert(lines, "      end")
    -- Now insert clipboard text at cursor, handling newlines
    table.insert(lines, "      local _pbytes = (@*[0]uint8)(_pclip)")
    table.insert(lines, "      local _pi: uint32 = 0")
    table.insert(lines, "      while _pbytes[_pi] ~= 0 and _pi < 4096 do")
    table.insert(lines, "        if _pbytes[_pi] == 10 then")
    table.insert(lines, "          -- newline: split current line")
    ...
    table.insert(lines, "        elseif _pbytes[_pi] ~= 13 then")
    table.insert(lines, "          -- regular byte: insert into current line")
    table.insert(lines, "          local _pline = self.visual.multi_buf:get_line(self.cursor_row)")
    table.insert(lines, "          while _pline:cursor() < self.cursor_col do _pline:move_cursor_right() end")
    table.insert(lines, "          while _pline:cursor() > self.cursor_col do _pline:move_cursor_left() end")
    table.insert(lines, "          _pline:insert_char(_pbytes[_pi])")
    ...
    -- ================================================================
    -- ★ Phase 4.8 S1: Ctrl+A — Select All
    -- ================================================================
    table.insert(lines, "  -- ★ Phase 4.8 S1: select all")
    table.insert(lines, "  if input.key_pressed == NebulaKey.SelectAll then")
    table.insert(lines, "    self.sel_anchor_row = 0; self.sel_anchor_col = 0")
    table.insert(lines, "    if self.line_count > 0 then")
    table.insert(lines, "      self.cursor_row = self.line_count - 1")
    table.insert(lines, "      self.cursor_col = (@uint32)(self.visual.multi_buf:get_line(self.cursor_row):len())")
    table.insert(lines, "    end")
    table.insert(lines, "  end")

    -- ================================================================
    -- ★ Phase 4.8 S1: Helper — delete selection before char input / Enter / Backspace / Delete
    ...
    table.insert(lines, "  if input.char_count > 0 and _ml_has_sel then")
    -- delete selection before char input
    _emit_ml_delete_selection(lines, "    ")
    table.insert(lines, "  end")

    -- Original char input logic
    ...
    table.insert(lines, "      -- ★ Phase 4.8 S1: reset anchor after char input")
    table.insert(lines, "      self.sel_anchor_row = self.cursor_row; self.sel_anchor_col = self.cursor_col")
    table.insert(lines, "    end")
    table.insert(lines, "    _ml_i = _ml_i + 1")
    table.insert(lines, "  end")

    -- ★ P1-7: wrap all key handling in a loop over key_input[0..key_count-1]
    table.insert(lines, "  local _ki: uint8 = 0")
    table.insert(lines, "  while _ki < input.key_count do")
    table.insert(lines, "    input.key_pressed = input.key_input[_ki]")
    -- Recompute selection state each iteration (previous key may have changed cursor/anchor)
    table.insert(lines, "    _ml_has_sel = (self.sel_anchor_row ~= self.cursor_row) or (self.sel_anchor_col ~= self.cursor_col)")
    table.insert(lines, "    if _ml_has_sel then")
    table.insert(lines, "      if self.sel_anchor_row < self.cursor_row or (self.sel_anchor_row == self.cursor_row and self.sel_anchor_col < self.cursor_col) then")
    table.insert(lines, "        _ml_sel_sr = self.sel_anchor_row; _ml_sel_sc = self.sel_anchor_col")
    table.insert(lines, "        _ml_sel_er = self.cursor_row; _ml_sel_ec = self.cursor_col")
    ...
    table.insert(lines, "      if _ml_has_sel then")
    -- delete selection before enter
    _emit_ml_delete_selection(lines, "        ")
    table.insert(lines, "      end")
    -- capture indent of current line before splitting
    table.insert(lines, "      do")
    table.insert(lines, "        local _ml_indent_line = self.visual.multi_buf:get_line(self.cursor_row)")
    table.insert(lines, "        local _ml_indent_tmp: [256]uint8")
    table.insert(lines, "        local _ml_indent_len = _ml_indent_line:flatten(&_ml_indent_tmp[0], 255)")
    table.insert(lines, "        local _ml_indent_n: uint32 = 0")
    table.insert(lines, "        while _ml_indent_n < (@uint32)(_ml_indent_len) and _ml_indent_tmp[_ml_indent_n] == 32 do")
    table.insert(lines, "          _ml_indent_n = _ml_indent_n + 1")
    table.insert(lines, "        end")
    -- check if last non-space char before cursor is { or :
    table.insert(lines, "        local _ml_extra_indent: boolean = false")
    table.insert(lines, "        if self.cursor_col > 0 then")
    table.insert(lines, "          local _ml_check_pos: uint32 = self.cursor_col - 1")
    -- skip trailing spaces before cursor
    table.insert(lines, "          while _ml_check_pos > 0 and _ml_indent_tmp[_ml_check_pos] == 32 do")
    table.insert(lines, "            _ml_check_pos = _ml_check_pos - 1")
    table.insert(lines, "          end")
    table.insert(lines, "          local _ml_last_ch = _ml_indent_tmp[_ml_check_pos]")
    table.insert(lines, "          if _ml_last_ch == 123 or _ml_last_ch == 58 then")  -- '{' = 123, ':' = 58
    table.insert(lines, "            _ml_extra_indent = true")
    table.insert(lines, "          end")
    table.insert(lines, "        end")
    -- perform the newline split
    table.insert(lines, "        self.visual.multi_buf.cursor_row = self.cursor_row")
    table.insert(lines, "        self.visual.multi_buf.cursor_col = self.cursor_col")
    table.insert(lines, "        self.visual.multi_buf:insert_newline()")
    table.insert(lines, "        self.cursor_row = self.visual.multi_buf.cursor_row")
    table.insert(lines, "        self.cursor_col = self.visual.multi_buf.cursor_col")
    table.insert(lines, "        self.line_count = self.visual.multi_buf.line_count")
    -- insert indent spaces on new line
    table.insert(lines, "        if _ml_extra_indent then _ml_indent_n = _ml_indent_n + 4 end")
    table.insert(lines, "        if _ml_indent_n > 0 then")
    table.insert(lines, "          local _ml_new_line = self.visual.multi_buf:get_line(self.cursor_row)")
    table.insert(lines, "          _ml_new_line:move_cursor_home()")
    table.insert(lines, "          local _ml_si: uint32 = 0")
    table.insert(lines, "          while _ml_si < _ml_indent_n do")
    table.insert(lines, "            _ml_new_line:insert_char(32)")
    table.insert(lines, "            _ml_si = _ml_si + 1")
    table.insert(lines, "          end")
    table.insert(lines, "          self.cursor_col = _ml_indent_n")
    table.insert(lines, "        end")
    table.insert(lines, "        self.sel_anchor_row = self.cursor_row; self.sel_anchor_col = self.cursor_col")
    table.insert(lines, "      end")
    table.insert(lines, "    end")
    ...
    -- ★ Phase 4.8-S5
    table.insert(lines, "    if input.key_pressed == NebulaKey.Tab then")
    table.insert(lines, "      if _ml_has_sel then")
    _emit_ml_delete_selection(lines, "        ")
    table.insert(lines, "      end")
    table.insert(lines, "      local _ml_tab_line = self.visual.multi_buf:get_line(self.cursor_row)")
    table.insert(lines, "      while _ml_tab_line:cursor() > self.cursor_col do _ml_tab_line:move_cursor_left() end")
    table.insert(lines, "      while _ml_tab_line:cursor() < self.cursor_col do _ml_tab_line:move_cursor_right() end")
    table.insert(lines, "      local _ml_ti: uint32 = 0")
    table.insert(lines, "      while _ml_ti < 4 do _ml_tab_line:insert_char(32); _ml_ti = _ml_ti + 1 end")
    table.insert(lines, "      self.cursor_col = self.cursor_col + 4")
    table.insert(lines, "      self.sel_anchor_row = self.cursor_row; self.sel_anchor_col = self.cursor_col")
    table.insert(lines, "    end")

    -- ---- ShiftTab: remove up to 4 leading spaces from current line ----
    -- ★ Phase 4.8-S5
    table.insert(lines, "    if input.key_pressed == NebulaKey.ShiftTab then")
    table.insert(lines, "      local _ml_st_line = self.visual.multi_buf:get_line(self.cursor_row)")
    table.insert(lines, "      local _ml_st_tmp: [256]uint8")
    table.insert(lines, "      local _ml_st_len = _ml_st_line:flatten(&_ml_st_tmp[0], 255)")
    -- count leading spaces (up to 4)
    table.insert(lines, "      local _ml_st_rm: uint32 = 0")
    table.insert(lines, "      while _ml_st_rm < 4 and _ml_st_rm < (@uint32)(_ml_st_len) and _ml_st_tmp[_ml_st_rm] == 32 do")
    table.insert(lines, "        _ml_st_rm = _ml_st_rm + 1")
    table.insert(lines, "      end")
    table.insert(lines, "      if _ml_st_rm > 0 then")
    table.insert(lines, "        _ml_st_line:move_cursor_home()")
    -- delete_range removes bytes [start, end)
    table.insert(lines, "        _ml_st_line:delete_range(0, _ml_st_rm)")
    table.insert(lines, "        if self.cursor_col >= _ml_st_rm then")
    table.insert(lines, "          self.cursor_col = self.cursor_col - _ml_st_rm")
    table.insert(lines, "        else")
    ...
    -- ---- Backspace: if selection active delete selection, else delete char before cursor ----
    table.insert(lines, "    if input.key_pressed == NebulaKey.Backspace then")
    table.insert(lines, "      if _ml_has_sel then")
    -- delete selection before backspace
    _emit_ml_delete_selection(lines, "        ")
    table.insert(lines, "      elseif self.cursor_col > 0 then")
    table.insert(lines, "        local _ml_line = self.visual.multi_buf:get_line(self.cursor_row)")
    table.insert(lines, "        while _ml_line:cursor() > self.cursor_col do _ml_line:move_cursor_left() end")
    table.insert(lines, "        while _ml_line:cursor() < self.cursor_col do _ml_line:move_cursor_right() end")
    ...
    -- ---- Delete: if selection active delete selection, else delete char at cursor ----
    table.insert(lines, "    if input.key_pressed == NebulaKey.Delete then")
    table.insert(lines, "      if _ml_has_sel then")
    -- delete selection before delete key
    _emit_ml_delete_selection(lines, "        ")
    table.insert(lines, "      else")
  ...
  for _, prim_name in ipairs(resolved) do
    ...
    if meta and meta.state_transitions then
      for _, tr in ipairs(meta.state_transitions) do
        ...
        if has(states, tr.target:lower()) then
...
local function _gen_process_text_input(lines, ctx)
  ...
  -- ★ P1-7: 控制键处理 — loop over key_input[0..key_count-1]
  table.insert(lines,  "  local _ki: uint8 = 0")
  table.insert(lines,  "  while _ki < input.key_count do")
  table.insert(lines,  "  local k = input.key_input[_ki]")
