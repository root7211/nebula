...
--
-- ★ P1-4 refactor: 管线类型注册表 (NEBULA_PIPELINES)
--   新增管线类型仅需调用 nebula_register_pipeline()，无需修改分发逻辑。
--
-- 公开 API：
--   nebula_register_pipeline(pspec)        -> void    (注册新管线类型)
...
local function gen_pipeline_shadow(base, uniforms_record, wgsl_source,
  ...
  -- ===== deinit（释放所有 GPU 资源，公理 B：L0 资源在 deinit 时销毁） =====
  -- ★ BUG-2 fix: 所有 release 调用前加 nilptr 守卫，防止 init() 部分失败后释放未初始化句柄
  ...
  -- 阴影遮罩管线
  emit("  if self.shadow_mask_pipeline ~= nilptr then wgpuRenderPipelineRelease(self.shadow_mask_pipeline) end")
  emit("  if self.shadow_mask_bg ~= nilptr then wgpuBindGroupRelease(self.shadow_mask_bg) end")
  emit("  if self.shadow_mask_bgl ~= nilptr then wgpuBindGroupLayoutRelease(self.shadow_mask_bgl) end")
  emit("  if self.shadow_mask_ubuf ~= nilptr then wgpuBufferRelease(self.shadow_mask_ubuf) end")
  -- 模糊管线 h
  emit("  if self.blur_h_pipeline ~= nilptr then wgpuRenderPipelineRelease(self.blur_h_pipeline) end")
  emit("  if self.blur_h_bg ~= nilptr then wgpuBindGroupRelease(self.blur_h_bg) end")
  emit("  if self.blur_h_bgl ~= nilptr then wgpuBindGroupLayoutRelease(self.blur_h_bgl) end")
  emit("  if self.blur_h_ubuf ~= nilptr then wgpuBufferRelease(self.blur_h_ubuf) end")
  -- 模糊管线 v
  emit("  if self.blur_v_pipeline ~= nilptr then wgpuRenderPipelineRelease(self.blur_v_pipeline) end")
  emit("  if self.blur_v_bg ~= nilptr then wgpuBindGroupRelease(self.blur_v_bg) end")
  emit("  if self.blur_v_bgl ~= nilptr then wgpuBindGroupLayoutRelease(self.blur_v_bgl) end")
  emit("  if self.blur_v_ubuf ~= nilptr then wgpuBufferRelease(self.blur_v_ubuf) end")
  -- 合成管线
  emit("  if self.composite_pipeline ~= nilptr then wgpuRenderPipelineRelease(self.composite_pipeline) end")
  emit("  if self.composite_bg ~= nilptr then wgpuBindGroupRelease(self.composite_bg) end")
  emit("  if self.composite_bgl ~= nilptr then wgpuBindGroupLayoutRelease(self.composite_bgl) end")
  emit("  if self.composite_ubuf ~= nilptr then wgpuBufferRelease(self.composite_ubuf) end")
  -- 离屏纹理
  emit("  if self.tex_a_view ~= nilptr then wgpuTextureViewRelease(self.tex_a_view) end")
  emit("  if self.tex_a ~= nilptr then wgpuTextureRelease(self.tex_a) end")
  emit("  if self.tex_b_view ~= nilptr then wgpuTextureViewRelease(self.tex_b_view) end")
  emit("  if self.tex_b ~= nilptr then wgpuTextureRelease(self.tex_b) end")
  -- 采样器
  emit("  if self.sampler ~= nilptr then wgpuSamplerRelease(self.sampler) end")
  emit("end")
...
local function gen_pipeline_standard_instanced(base, uniforms_record, wgsl_source, max_instances)
  ...
  emit("    usage            = (@uint64)(WGPUBufferUsage_Uniform) | (@uint64)(WGPUBufferUsage_CopyDst),")
  emit("    size             = 16,")
  emit("    mappedAtCreation = 0,")
  emit("  }")
  ...
  emit("    usage            = (@uint64)(WGPUBufferUsage_Storage) | (@uint64)(WGPUBufferUsage_CopyDst),")
  ...
  emit("                    stripIndexFormat = 0, frontFace = 0, cullMode = 0, unclippedDepth = 0 },")
  else
  emit("    primitive   = { nextInChain = nilptr, topology = WGPUPrimitiveTopology_TriangleList,")
  emit("                    stripIndexFormat = 0, frontFace = 0, cullMode = 0 },")
  end
  emit("    depthStencil = nilptr,")
  emit("    multisample  = { nextInChain = nilptr, count = 1, mask = 0xFFFFFFFF, alphaToCoverageEnabled = 0 },")
...
-- =============================================================================
-- ★ P1-4 refactor: NEBULA_PIPELINES 数据驱动注册表
--
-- 每种管线类型注册自己的 flag 名称、必需字段、生成器函数。
-- nebula_gen_pipeline_source 通过遍历注册表分发，新增管线类型仅需
-- 在此处添加一行 nebula_register_pipeline() 调用。
--
-- 对标已有的 NEBULA_PRIMITIVES 注册表设计模式。
-- =============================================================================
local NEBULA_PIPELINES = {}

-- nebula_register_pipeline(spec)
-- 注册一种管线类型到全局注册表。
--   spec.flag           : string  — spec 中的布尔标志名（如 "has_shadow"）
--   spec.required_fields: table   — 该路径需要的额外 spec 字段名列表
--   spec.generator      : function(spec) -> string — 代码生成器
--   spec.description    : string  — 可读描述（调试用）
function nebula_register_pipeline(pspec)
  assert(pspec.flag,      "nebula_register_pipeline: flag required")
  assert(pspec.generator, "nebula_register_pipeline: generator required")
  pspec.required_fields = pspec.required_fields or {}
  pspec.description     = pspec.description or pspec.flag
  table.insert(NEBULA_PIPELINES, pspec)
end

-- ===== 注册内置管线类型 =====

nebula_register_pipeline({
  flag = "has_shadow",
  description = "Shadow multi-pass pipeline (Phase 2.5)",
  required_fields = { "shadow_mask_source", "blur_h_source", "blur_v_source", "composite_source" },
...
nebula_register_pipeline({
  flag = "standard_instanced",
  description = "Standard instanced pipeline (Phase 3.5.1)",
  required_fields = { "wgsl_source" },
  generator = function(spec)
...
nebula_register_pipeline({
  flag = "textured",
  description = "Textured vertex pipeline — SDF text (Phase 3.2.2)",
  required_fields = { "wgsl_source" },
  generator = function(spec)
...
nebula_register_pipeline({
  flag = "slug_text",
  description = "Slug vector text pipeline (Phase 4.1)",
  required_fields = { "wgsl_source" },
  generator = function(spec)
...
nebula_register_pipeline({
  flag = "atlas_dense",
  description = "Dense text instanced pipeline — SDF atlas (Phase 4.X)",
  required_fields = { "wgsl_source" },
  generator = function(spec)
...
--
-- ★ P1-4 refactor: 数据驱动分发，替代原来的 5 路 if/elseif 硬编码链。
-- 新增管线类型仅需调用 nebula_register_pipeline()，无需修改此函数。
-- =============================================================================
function nebula_gen_pipeline_source(spec)
  ...
  -- 互斥标志校验——注册表中的路径最多只能激活一条
  local _active = {}
  for _, pspec in ipairs(NEBULA_PIPELINES) do
    if spec[pspec.flag] then _active[#_active + 1] = pspec.flag end
  end
  if #_active > 1 then
  ...
  -- 数据驱动分发：遍历注册表，匹配激活的 flag
  for _, pspec in ipairs(NEBULA_PIPELINES) do
    if spec[pspec.flag] then
      -- 校验必需字段
      for _, field in ipairs(pspec.required_fields) do
        assert(spec[field],
          ("nebula_gen_pipeline_source: %s required when %s is set (pipeline: %s)"):format(
            field, pspec.flag, spec.base))
      end
      return pspec.generator(spec)
    end
  end
  ...
  local known_flags = {}
  for _, pspec in ipairs(NEBULA_PIPELINES) do
    table.insert(known_flags, pspec.flag)
  end
  error("nebula_gen_pipeline_source: unrecognized spec for '" .. tostring(spec.base) .. "'. " ..
        "Set one of: " .. table.concat(known_flags, ", ") .. ". " ..
        "Or register a custom pipeline type via nebula_register_pipeline().")
end
