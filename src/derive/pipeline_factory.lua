...
-- ===== P2-2 helper: 生成 WASM/native RenderPassColorAttachment + Descriptor =====
-- 消除 draw_shadow 中 6 处重复的 attachment 构造代码。
-- view_expr: Nelua 表达式字符串（如 "self.tex_a_view"）
local function emit_render_pass_begin(emit, view_expr)
  if NEBULA_TARGET ~= 'wasm' then
  emit("    local att = WGPURenderPassColorAttachment{")
  emit("      nextInChain = nilptr,")
  emit(("      view        = %s,"):format(view_expr))
...
local function gen_pipeline_shadow(base, uniforms_record, wgsl_source,
  ...
  emit("  do")
  emit_render_pass_begin(emit, "self.tex_a_view")
  ...
  emit_render_pass_begin(emit, "self.tex_b_view")
