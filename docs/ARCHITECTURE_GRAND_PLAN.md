...
| Phase | 名称 | 目标 | 状态 |
| :--- | :--- | :--- | :--- |
| **4.2.1** | 跨平台 PAL 骨架 | 源码级支持 Linux/Windows/Web，实现多端对齐。 | 已完成 |
| **4.2.2** | Slug 渲染内核生产级化 | 清算 MVP 债务，支持任意仿射变换与 Latin 全量字符。 | D-4.1-A/B 已完成，D-4.1-C 待评估 |
| **4.2.3** | HarfBuzz + CJK 集成 | 解决复杂文本排版与中文字形规模化问题。 | 规划中 |
| **4.3** | **可编程原语注册表** | **核心功能：允许开发者安全注入自定义交互逻辑。** | 规划中 |
| 4.4 | 高级组件库 | 实现 `scrollable`、`multiline_edit`、`dropdown` 等标准组件。 | 规划中 |
| 4.5 | 工业级应用原型 | 构建一个功能完备的 IDE 级别文本编辑器作为里程碑。 | 规划中 |
| 5.0 | 生态与 CI/CD | 完善文档、包管理支持与多端自动化测试。 | 规划中 |
